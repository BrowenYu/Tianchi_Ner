...
import collections
import numpy as np
from flat_utils import size2MB, MyDropout
...
class MultiHead_Attention_Lattice_rel_save_gpumm(nn.Module):
    ...
    def forward(self, key, query, value, seq_len, lex_num, pos_s, pos_e, rel_pos_embedding):
        ...
        mask = seq_len_to_mask(seq_len + lex_num).byte()
        mask = torch.cat([mask, torch.zeros(max_seq_len - len(mask[0]), dtype=torch.uint8).unsqueeze(0)], dim=-1)
...
class MultiHead_Attention_Lattice_rel(nn.Module):
    ...
    def forward(self, key, query, value, seq_len, lex_num, pos_s, pos_e):
        ...
        # print(f'pe_ss:{pos_ss.view(-1) + self.max_seq_len}')
        # print(f'pe_se:{pos_se.view(-1) + self.max_seq_len}')
        # print(f'pe_es:{pos_es.view(-1) + self.max_seq_len}')
        # print(f'pe_ee:{pos_ee.view(-1) + self.max_seq_len}')
        ...
        # print('pe_ss:{}'.format(pe_ss.size()))
        # print('pe_se:{}'.format(pe_se.size()))
        # print('pe_es:{}'.format(pe_es.size()))
        # print('pe_ee:{}'.format(pe_ee.size()))
        ...
        # A, B C and D is exactly the shape
        if self.mode['debug']:
            print('A:{}'.format(A_.size()))
            print('B:{}'.format(B_.size()))
            print('C:{}'.format(C_.size()))
            print('D:{}'.format(D_.size()))
        attn_score_raw = A_ + B_ + C_ + D_

        # print(f'attn_score_raw:{attn_score_raw.shape}')
        ...
        # print(seq_len, lex_num)
        # print(mask.shape, attn_score_raw.shape)
...
class Absolute_Position_Embedding(nn.Module):
    def __init__(self, hidden_size, max_len=5000, learnable=False, mode=collections.defaultdict(bool), pos_norm=False):
        ...
        if self.mode['debug']:
            ...
            print('pe avg:{}'.format(torch.sum(self.pe) / (self.pe.size(2) * self.pe.size(1))))

    def forward(self, inp):
        if self.mode['debug']:
            print('now in Absolute Position Embedding')
...
class Transformer_Encoder_Layer(nn.Module):
    def __init__(self, hidden_size, num_heads,
        ...
        self.layer_preprocess = Layer_Process(self.layer_preprocess_sequence, self.hidden_size, self.dropout)
        self.layer_postprocess = Layer_Process(self.layer_postprocess_sequence, self.hidden_size, self.dropout)
        if self.relative_position:
            if not self.lattice:
